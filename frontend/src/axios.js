...
export const fetchPost = (keyword, currentPage, price, category) => {
	if (category) {
		return API.get(
			`/api/v1/products?keyword=${keyword}&page=${currentPage}&price[gt]=${price[0]}&price[lt]=${price[1]}&category=${category}`,
		);
	}
	return API.get(
